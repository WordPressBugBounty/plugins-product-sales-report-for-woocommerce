...
Perfect for smart decision making, monitoring sales performance, order fulfillment, inventory management, and accounting. Works with both WooCommerce and Easy Digital Downloads (beta).
...
- **Blank Product Report**: A minimal starting point for a product report - pick your own fields and settings from scratch.
- **All Sales**: Sales trend chart with quantity sold and revenue after discounts, shown per product.
- **Top Selling Products**: Identify top selling products with quantities and revenue in a single view.
- **Top Rated Products**: See products with the highest customer ratings to highlight quality performers. (Pro only)
- **Sales by Product**: Review sales totals grouped by product to spot trends quickly.
- **Refunds By Product**: Top products by refunded amount in the period. Compare with sales columns; add a calculated field for refund rate if you need it. (Pro only)

#### Inventory Management Templates:
- **Stock Report**: Monitor stock levels and surface products that need replenishment.
- **Low & Out Of Stock**: Lowest on-hand quantities first so shortages stand out; sold units follow the report dates and statuses.

#### Geographic & Segmentation Templates:
- **Sales by Country**: Analyse performance by billing country for geo insights.
- **Sales by Currency**: Track sales totals by currency when selling internationally.
- **Sales by Payment Method**: Compare revenue by payment method to understand buyer preferences.
- **Sales Trend (Monthly)**: Revenue and units sold summed by calendar month across the dates you choose.
- **Sales Trend (Weekly)**: Revenue and units sold summed by week across the dates you choose.
- **Top Revenue By Billing Email**: Ranking of billing emails by revenue in your date range, with units, orders and average order total (top 100 rows).
- **Sales by US State**: Break down sales totals by US state for regional insights. (Pro only)
- **Refund Trend (Monthly)**: Refunded amounts and units by calendar month across the dates you choose. (Pro only)

#### Order Export Templates:
- **Blank Order Export**: A minimal starting point for an order export - choose your own fields, one row per order or per line item.
- **Accounting**: One row per completed order with totals, tax, and billing country for bookkeeping.
- **Fulfillment & Shipping**: One row per order with shipping contact, address, method and quantities for picking, packing and labels.
- **Refunds & Returns**: One row per refunded order with status, and amounts.
- **Line Items (One Row Per Item)**: One row per line item for supplier files, picking lists and SKU-level analysis.
- **Order Margin Summary**: One row per completed order with revenue and tax amounts. COGS columns appear when cost tracking is enabled.
...
[Frontend Reports](https://wpzone.co/product/frontend-reports-for-woocommerce/) - embed a report or a download link in a post or a page
[Export Order Items Pro](https://wpzone.co/product/export-order-items-pro-for-woocommerce/) - export order details for use in order fulfillment and data analysis
...
1. Reports list - all your saved reports in one place
2. Create Report - start from a template or a blank report, grouped by integration
3. Choose Report Template - templates grouped by type, with Pro options clearly marked
4. Settings - general options and live preview customization (quick links, status colors, negative number highlighting)
5. Report editor (table view) - Top Selling Products with clickable order/product links
6. Report editor (line chart) - monthly sales trend for revenue and units sold
7. Report editor (bar chart) - top revenue by billing email
8. Same report in table view, with sortable columns
9. Order Margin Summary export - negative amounts highlighted in red
10. Date range picker - Quick presets with an advanced "date range based on" option
11. Quick date presets - Last 7/30/90 days, this month, last year, and more
12. Relative date range - e.g. "12 months ago" to "1 month ago"
13. Absolute date range - exact from/to dates and times
14. Stock Report - product filtering options, including excluding items without stock management
15. Accounting export - Report Fields panel with the full field picker open
16. Segmentation - group results by a custom field alongside the main segment
17. Orders panel - filter by order status, with advanced Pro filtering options
18. Debug tab - inspect the exact SQL queries behind a report
